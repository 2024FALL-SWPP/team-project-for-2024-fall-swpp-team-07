...
public class Stage3Manager : StageManager
{
    ...
    public GameObject[] almonds = new GameObject[5]; // stage 3에서는 아몬드 5개
    public GameObject[] almondUI = new GameObject[5]; // stage 3에서는 아몬드 5개
    public GameObject[] almondUI_disabled = new GameObject[5]; // stage 3에서는 아몬드 5개
    private bool[] almondStatus = new bool[5]; // 아몬드 획득 여부
    ...
    private async Task updateStageRecord(float finalTime, int numHits)
    {
        try
        {
            ...
            if (client != null && client.Auth.CurrentSession != null)
            {
                ...
                // 일단 stage3_clear를 true로 업데이트
                await client
                    .From<UserProfile>()
                    .Where(x => x.user_id == userId)
                    .Set(x => x.stage3_clear, true)
                    .Update();
