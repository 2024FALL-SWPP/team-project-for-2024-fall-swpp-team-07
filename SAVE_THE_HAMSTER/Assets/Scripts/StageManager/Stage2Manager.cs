...
{
    // This script takes care of all necessary info for stage 2.
    private int stageIndex = 1;
    public GameObject[] almonds = new GameObject[4]; // stage 2에서는 아몬드 4개
    public GameObject[] almondUI = new GameObject[4]; // stage 2에서는 아몬드 4개
    public GameObject[] almondUI_disabled = new GameObject[4]; // stage 2에서는 아몬드 4개
    private bool[] almondStatus = new bool[4]; // 아몬드 획득 여부
    ...
    private async Task updateStageRecord(float finalTime, int numHits)
    {
        try
        {
            ...
            if (client != null && client.Auth.CurrentSession != null)
            {
                ...
                // 일단 stage2_clear를 true로 업데이트
                await client
                    .From<UserProfile>()
                    .Where(x => x.user_id == userId)
                    .Set(x => x.stage2_clear, true)
                    .Update();
