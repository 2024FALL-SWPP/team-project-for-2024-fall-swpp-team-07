...
public class CPC_CameraPathInspector : Editor
{
    ...
    //GUIContents
    private GUIContent addPointContent = new GUIContent(
        "Add Point",
        "Adds a waypoint at the scene view camera's position/rotation"
    );
    private GUIContent testButtonContent = new GUIContent("Test", "Only available in play mode");
    private GUIContent pauseButtonContent = new GUIContent(
        "Pause",
        "Paused Camera at current Position"
    );
    private GUIContent continueButtonContent = new GUIContent(
        "Continue",
        "Continues Path at current position"
    );
    ...
    private GUIContent gotoPointContent = new GUIContent(
        "Goto",
        "Teleports the scene camera to the specified waypoint"
    );
    private GUIContent relocateContent = new GUIContent(
        "Relocate",
        "Relocates the specified camera to the current view camera's position/rotation"
    );
    private GUIContent alwaysShowContent = new GUIContent(
        "Always show",
        "When true, shows the curve even when the GameObject is not selected - \"Inactive cath color\" will be used as path color instead"
    );
    private GUIContent chainedContent = new GUIContent(
        "o───o",
        "Toggles if the handles of the specified waypoint should be chained (mirrored) or not"
    );
    private GUIContent unchainedContent = new GUIContent(
        "o─x─o",
        "Toggles if the handles of the specified waypoint should be chained (mirrored) or not"
    );
    private GUIContent replaceAllPositionContent = new GUIContent(
        "Replace all position lerps",
        "Replaces curve types (and curves when set to \"Custom\") of all the waypoint position lerp types with the specified values"
    );
    private GUIContent replaceAllRotationContent = new GUIContent(
        "Replace all rotation lerps",
        "Replaces curve types (and curves when set to \"Custom\") of all the waypoint rotation lerp types with the specified values"
    );
    ...
    {
        cameraTranslateMode = (CPC_EManipulationModes)
            PlayerPrefs.GetInt("CPC_cameraTranslateMode", 1);
        cameraRotationMode = (CPC_EManipulationModes)
            PlayerPrefs.GetInt("CPC_cameraRotationMode", 1);
        handlePositionMode = (CPC_EManipulationModes)
            PlayerPrefs.GetInt("CPC_handlePositionMode", 0);
    ...
    void GetVariableProperties()
    {
        ...
        visualInactivePathProperty = serializedObjectTarget.FindProperty(
            "visual.inactivePathColor"
        );
    ...
    {
        pointReorderableList = new ReorderableList(
            serializedObject,
        ...
        pointReorderableList.drawElementCallback = (rect, index, active, focused) =>
        {
            ...
            CPC_ECurveType tempP = (CPC_ECurveType)
                EditorGUI.EnumPopup(rect, t.points[index].curveTypePosition);
            ...
            rect.width = (fullWidth - 22) / 3 - 1;
            EditorGUI.BeginChangeCheck();
            CPC_ECurveType temp = (CPC_ECurveType)
                EditorGUI.EnumPopup(rect, t.points[index].curveTypeRotation);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(t, "Changed enum value");
            ...
            if (GUI.Button(rect, relocateContent))
            {
                ...
                pointReorderableList.index = index;
                selectedIndex = index;
                t.points[pointReorderableList.index].position = SceneView
                    .lastActiveSceneView
                    .camera
                    .transform
                    .position;
                t.points[pointReorderableList.index].rotation = SceneView
                    .lastActiveSceneView
                    .camera
                    .transform
                    .rotation;
    ...
    void DrawPlaybackWindow()
    {
        ...
        GUI.enabled = Application.isPlaying;
        EditorGUI.BeginChangeCheck();
        currentTime = EditorGUILayout.Slider(
            currentTime,
            0,
            t.points.Count - ((t.looped) ? 0.01f : 1.01f)
        );
        if (EditorGUI.EndChangeCheck())
        {
    ...
    {
        GUILayout.BeginHorizontal();
        useMainCameraProperty.boolValue = GUILayout.Toggle(
            useMainCameraProperty.boolValue,
            "Use main camera",
            GUILayout.Width(Screen.width / 3f)
        );
        GUI.enabled = !useMainCameraProperty.boolValue;
        selectedCameraProperty.objectReferenceValue = (Camera)
            EditorGUILayout.ObjectField(
                selectedCameraProperty.objectReferenceValue,
                typeof(Camera),
                true
            );
        GUI.enabled = true;
        ...
        GUILayout.BeginHorizontal();
        lookAtTargetProperty.boolValue = GUILayout.Toggle(
            lookAtTargetProperty.boolValue,
            "Look at target",
            GUILayout.Width(Screen.width / 3f)
        );
        GUI.enabled = lookAtTargetProperty.boolValue;
        lookAtTargetTransformProperty.objectReferenceValue = (Transform)
            EditorGUILayout.ObjectField(
                lookAtTargetTransformProperty.objectReferenceValue,
                typeof(Transform),
                true
            );
        GUI.enabled = true;
        ...
        GUILayout.BeginHorizontal();
        loopedProperty.boolValue = GUILayout.Toggle(
            loopedProperty.boolValue,
            "Looped",
            GUILayout.Width(Screen.width / 3f)
        );
        GUI.enabled = loopedProperty.boolValue;
        GUILayout.Label("After loop:", GUILayout.Width(Screen.width / 4f));
        afterLoopProperty.enumValueIndex = Convert.ToInt32(
            EditorGUILayout.EnumPopup((CPC_EAfterLoop)afterLoopProperty.intValue)
        );
        GUI.enabled = true;
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        playOnAwakeProperty.boolValue = GUILayout.Toggle(
            playOnAwakeProperty.boolValue,
            "Play on awake",
            GUILayout.Width(Screen.width / 3f)
        );
        GUI.enabled = playOnAwakeProperty.boolValue;
        GUILayout.Label("Time: ", GUILayout.Width(Screen.width / 4f));
        playOnAwakeTimeProperty.floatValue = EditorGUILayout.FloatField(
            playOnAwakeTimeProperty.floatValue
        );
        GUI.enabled = true;
        GUILayout.EndHorizontal();
    }
    ...
    void DrawVisualDropdown()
    {
        ...
        alwaysShowProperty.boolValue = GUILayout.Toggle(
            alwaysShowProperty.boolValue,
            alwaysShowContent
        );
        GUILayout.EndHorizontal();
        if (visualFoldout)
        {
            GUILayout.BeginVertical("Box");
            visualPathProperty.colorValue = EditorGUILayout.ColorField(
                "Path color",
                visualPathProperty.colorValue
            );
            visualInactivePathProperty.colorValue = EditorGUILayout.ColorField(
                "Inactive path color",
                visualInactivePathProperty.colorValue
            );
            visualFrustumProperty.colorValue = EditorGUILayout.ColorField(
                "Frustum color",
                visualFrustumProperty.colorValue
            );
            visualHandleProperty.colorValue = EditorGUILayout.ColorField(
                "Handle color",
                visualHandleProperty.colorValue
            );
    ...
    {
        manipulationFoldout = EditorGUILayout.Foldout(
            manipulationFoldout,
        ...
        if (manipulationFoldout)
        {
            GUILayout.BeginVertical("Box");
            cameraTranslateMode = (CPC_EManipulationModes)
                EditorGUILayout.EnumPopup("Waypoint Translation", cameraTranslateMode);
            cameraRotationMode = (CPC_EManipulationModes)
                EditorGUILayout.EnumPopup("Waypoint Rotation", cameraRotationMode);
            handlePositionMode = (CPC_EManipulationModes)
                EditorGUILayout.EnumPopup("Handle Translation", handlePositionMode);
            GUILayout.EndVertical();
        }
        if (EditorGUI.EndChangeCheck())
        {
    ...
    void DrawWaypointList()
    {
        ...
        GUILayout.BeginVertical("Box");
        GUILayout.BeginHorizontal();
        allCurveType = (CPC_ECurveType)
            EditorGUILayout.EnumPopup(allCurveType, GUILayout.Width(Screen.width / 3f));
        ...
        allAnimationCurve = EditorGUILayout.CurveField(
            allAnimationCurve,
            GUILayout.Width(Screen.width / 3f)
        );
        GUI.enabled = true;
            ...
            switch (waypointMode)
            {
                ...
                case CPC_ENewWaypointMode.LastWaypoint:
                    if (t.points.Count > 0)
                        t.points.Add(
                            new CPC_Point(
                                t.points[t.points.Count - 1].position,
                                t.points[t.points.Count - 1].rotation
                            )
                            {
                                handlenext = t.points[t.points.Count - 1].handlenext,
                                handleprev = t.points[t.points.Count - 1].handleprev,
                            }
                        );
                    else
                    {
                        ...
                        Debug.LogWarning(
                            "No previous waypoint found to place this waypoint, defaulting position to world center"
                        );
                    }
                    break;
                case CPC_ENewWaypointMode.WaypointIndex:
                    if (t.points.Count > waypointIndex - 1 && waypointIndex > 0)
                        t.points.Add(
                            new CPC_Point(
                                t.points[waypointIndex - 1].position,
                                t.points[waypointIndex - 1].rotation
                            )
                            {
                                handlenext = t.points[waypointIndex - 1].handlenext,
                                handleprev = t.points[waypointIndex - 1].handleprev,
                            }
                        );
                    else
                    {
                        ...
                        Debug.LogWarning(
                            "Waypoint index "
                                + waypointIndex
                                + " does not exist, defaulting position to world center"
                        );
                    }
                    break;
        ...
        EditorGUI.BeginChangeCheck();
        waypointMode = (CPC_ENewWaypointMode)
            EditorGUILayout.EnumPopup(
                waypointMode,
                waypointMode == CPC_ENewWaypointMode.WaypointIndex
                    ? GUILayout.Width(Screen.width / 4)
                    : GUILayout.Width(Screen.width / 2)
            );
        if (waypointMode == CPC_ENewWaypointMode.WaypointIndex)
        {
            waypointIndex = EditorGUILayout.IntField(
                waypointIndex,
                GUILayout.Width(Screen.width / 4)
            );
        }
    ...
    void DrawNextHandle(int i)
    {
        if (i < t.points.Count - 1 || loopedProperty.boolValue)
        {
            ...
            float size =
                HandleUtility.GetHandleSize(t.points[i].position + t.points[i].handlenext) * 0.1f;
            if (handlePositionMode == CPC_EManipulationModes.Free)
            {
#if UNITY_5_5_OR_NEWER
                posNext = Handles.FreeMoveHandle(
                    t.points[i].position + t.points[i].handlenext,
                    Quaternion.identity,
                    size,
                    Vector3.zero,
...
#endif
                    posNext = Handles.PositionHandle(
                        t.points[i].position + t.points[i].handlenext,
                        Quaternion.identity
                    );
                }
    ...
    void DrawPrevHandle(int i)
    {
        if (i > 0 || loopedProperty.boolValue)
        {
            ...
            float size =
                HandleUtility.GetHandleSize(t.points[i].position + t.points[i].handleprev) * 0.1f;
            if (handlePositionMode == CPC_EManipulationModes.Free)
            {
#if UNITY_5_5_OR_NEWER
                posPrev = Handles.FreeMoveHandle(
                    t.points[i].position + t.points[i].handleprev,
                    Quaternion.identity,
                    0.1f
...
#endif
                    posPrev = Handles.PositionHandle(
                        t.points[i].position + t.points[i].handleprev,
                        Quaternion.identity
                    );
                }
            ...
            else
            {
                ...
                pos = Handles.FreeMoveHandle(
                    ...
                    (Tools.pivotRotation == PivotRotation.Local)
                        ? t.points[i].rotation
                        : Quaternion.identity,
                    HandleUtility.GetHandleSize(t.points[i].position) * 0.2f,
                    Vector3.zero,
    ...
    void DrawSelectionHandles(int i)
    {
        ...
        {
            if (
                cameraTranslateMode == CPC_EManipulationModes.SelectAndTransform
                    && Tools.current == Tool.Move
                || cameraRotationMode == CPC_EManipulationModes.SelectAndTransform
        ...
        if (showRawValues)
        {
            foreach (var i in t.points)
            {
                ...
                Quaternion rot = Quaternion.Euler(
                    EditorGUILayout.Vector3Field("Waypoint Rotation", i.rotation.eulerAngles)
                );
