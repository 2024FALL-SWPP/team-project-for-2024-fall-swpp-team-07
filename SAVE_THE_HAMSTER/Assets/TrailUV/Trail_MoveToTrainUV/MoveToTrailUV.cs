...
public class MoveToTrailUV : MonoBehaviour
{
    ...
    public GameObject[] m_targetsToFollow;
    ...
    public MaterialData[] m_materialData = new MaterialData[1]
    {
        new MaterialData(null, null, new Vector2(1, 1), 0f),
    };
    ...
    {
        Initialize();
        m_targetsToFollow = FindObjectsOfLayer(LayerMask.NameToLayer("Ball"));
    }
    ...
    void LateUpdate()
    {
        ...
        if (m_targetsToFollow != null)
        {
            // m_targetsToFollow 중에서 active한 것을 찾아서 위치를 복사
            foreach (GameObject target in m_targetsToFollow)
            {
                if (target.activeSelf)
                {
                    m_moveObject.position = target.transform.position;
                    break;
                }
            }
        }
