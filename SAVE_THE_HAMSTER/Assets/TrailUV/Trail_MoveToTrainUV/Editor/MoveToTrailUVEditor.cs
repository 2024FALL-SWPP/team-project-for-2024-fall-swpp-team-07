...
[CustomEditor(typeof(MoveToTrailUV))]
...
public class MoveToTrailUVEditor : Editor
{
    ...
    private void OnDisable()
    {
        ...
        for (int i = 0; i < m_materialData_sp.arraySize; i++)
        {
            ...
            TrailRenderer trailRenderer = (TrailRenderer)
                materialDataElement_sp.FindPropertyRelative("m_trailRenderer").objectReferenceValue;
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (checkTrailRenderertile != -1)
        {
            string message = String.Format(
                "Element {0}번 Trail Renderer의 Texture Mode가 Tile이 아닙니다.",
                checkTrailRenderertile
            );
    ...
    private void SyncElementTiling(SerializedProperty materialDataElement_sp) // 재질의 타일링만 동기화
    {
        ...
        if (trailRenderer != null)
        {
            ...
            {
                if (
                    materialDataElement_sp.FindPropertyRelative("m_uvTiling").vector2Value
    ...
    {
        if (
            m_materialData_sp.serializedObject.targetObject == null
            || m_materialData_sp.arraySize == 0
        )
            return;
